...
<head>
    ...
    <style>
        ...
        .cell { width: 60px; height: 60px; background-color: var(--cell-bg); display: flex; justify-content: center; align-items: center; box-sizing: border-box; border: 1px solid #777; position: relative; transition: background-color 0.2s; }
        ...
        .highlight-move { background-color: var(--highlight-move); }
        .highlight-attack { background-color: var(--highlight-attack); }
        .highlight-placement { background-color: var(--highlight-placement); border: 2px dashed #3498db; }
        .player1-crown-spot, .player2-crown-spot { background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">👑</text></svg>'); background-size: 70%; background-repeat: no-repeat; background-position: center; background-color: #FFD700; opacity: 0.5; }
...
<body>
    ...
    <div id="dice-modal" class="modal">
        <div id="dice-modal-content" class="modal-content">
            <h3 id="dice-title">Rolling...</h3>
            <div id="dice-container"><svg id="dice-svg" viewBox="0 0 100 100"><polygon class="face" points="50,1 95,25 95,75 50,99 5,75 5,25" /><text class="text" x="50" y="52">20</text></svg></div>
            <div id="dice-results"></div>
        </div>
    </div>

    <div id="victoryModal" class="modal">
        <div id="victoryContent" class="modal-content">
            <div id="loserAnimation" style="display: none;">
    ...
    <div id="crownChoiceModal" class="modal"><div id="crownChoiceContent" class="modal-content"><h3>You've Reached the Crown Space!</h3><p>Choose your action. You have <b id="crown-modal-reserve-count">X</b> pieces in reserve.</p><div id="crownChoiceActions"></div></div></div>
    <!-- New Modal for Reinforcement Choice -->
    <div id="reinforceChoiceModal" class="modal"><div id="reinforceChoiceContent" class="modal-content"><h3>Choose Reinforcement Location</h3><div id="reinforceChoiceActions"></div></div></div>
    ...
    <script>
        ...
        function isIllegalHomeRow(player, targetRow) {
            const p1HomeRow = gameState.boardSize.rows - 1;
            const p2HomeRow = 0;
            if (player === 'p1' && targetRow === p1HomeRow) return true;
            if (player === 'p2' && targetRow === p2HomeRow) return true;
            return false;
        }

        function movePieceOnBoard(piece, newR, newC) {
            if (!piece) return;
            if (isIllegalHomeRow(piece.owner, newR)) {
                console.error(`CRITICAL ERROR: An illegal move to home row ${newR} was attempted for Player ${piece.owner} and blocked.`);
                return;
            }
        ...
        function initializeGame() {
            ...
            gameState = {
                ...
                pieces: [],
                players: {
                    p1: { id: 'p1', color: null, dieType: null, piecesInReserve: 16, capturedOpponentPieces: 0 },
                    p2: { id: 'p2', color: null, dieType: null, piecesInReserve: 16, <!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <script>
        ...
        function initializeGame() {
            ...
            gameState = {
                ...
                players: {
                    p1: { id: 'p1', color: null, dieType: null, piecesInReserve: 16, capturedOpponentPieces: 0 },
                    p2: { id: 'p2', color: null, dieType: null, piecesInReserve: 16, capturedOpponentPieces: 0 }
                },
                crownSpaces: { p1: { row: 4, col: 3 }, p2: { row: 0, col: 3 } },
                turn: null, gamePhase: 'SETUP_D20_ROLL', winner:capturedOpponentPieces: 0 }
                },
                crownSpaces: { p1: { row: 4, col: 3 }, p2: { row: 0, col: 3 } },
                turn: null, gamePhase: 'SETUP_D20_ROLL', winner:
